...
class WSKafkaConnector extends EventEmitter {
    ...
    initConsumerOrSubscribe(ws, msg) {
        ...
        client.loadMetadataForTopics(topic_names, function (e, res){
            if (e) {
                let rm = Msg.createReplyMessage(msg);
                rm.status = Msg.STATUS_FAIL;
                rm.payload = e;
                ...
                topicsPayload = [];
                topic_names.forEach(tn => {
                   if(res[1].metadata[tn]){
                       let partitions = Object.keys(res[1].metadata[tn]);
                       partitions.forEach(p => { topicsPayload.push({topic:tn, partition:parseInt(p)}) })
                   }
                });
            }
            this.createConsumer(ws, client, topicsPayload, msg);
        }.bind(this)
    );
